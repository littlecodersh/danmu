...
class BilibiliDanMuClient(AbstractDanMuClient):
    def _get_live_status(self):
        ...
        self.roomId = re.findall(b'var ROOMID = (\d+);', requests.get(url).content)[0]
        r = requests.get('http://live.bilibili.com/api/player?id=cid:' + self.roomId)
        self.serverUrl = re.findall(b'<server>(.*?)</server>', r.content)[0]
        return re.findall(b'<state>(.*?)</state>', r.content)[0] == 'LIVE'
